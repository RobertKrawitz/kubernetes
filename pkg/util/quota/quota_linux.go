...
func detectSupportsQuotas(path string) (bool, error) {
	...
	if errno == 0 {
		if qstat.qs_flags&C.FS_QUOTA_PDQ_ENFD > 0 && qstat.qs_flags&C.FS_QUOTA_PDQ_ACCT > 0 {
			glog.V(3).Infof("detectSupportQuota %s YES", path)
			return true, nil
		} else {
			glog.V(3).Infof("detectSupportQuota %s NO", path)
			return false, nil
		}
	}
	glog.V(3).Infof("detectSupportQuota %s FAILED %v", path, errno)
	return false, errno
}
...
func AssignQuota(m mount.Interface, path string, poduid string, bytes int64) (QuotaID, error) {
	...
	glog.V(3).Infof("Synthesizing pod ID %s for directory %s in AssignQuota", poduid, path)
	...
	for id := firstQuota; id == id; id++ {
		...
		if idIsInUse(path, id) {
			glog.V(3).Infof("Project ID %v is in use, try again", id)
			continue
		}
		err := setQuotaOn(path, id, bytes)
		if err != nil {
