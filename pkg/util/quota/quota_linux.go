...
func detectBackingDev(m mount.Interface, path string) (error) {
	...
	for scanner.Scan() {
		...
		if match != nil {
			device := match[1]
			mount := match[2]
			glog.V(3).Infof("detectBackingDev looking for mountpoint %s %s", mountpoint, mount)
			if mount == mountpoint {
				glog.V(3).Infof("Found device for %s: %s", path, device)
				backingDevsMap[path] = device
				return nil
			}
		}
	}
	glog.V(3).Infof("Couldn't find backing device for %s!", path)
	return nil
}
...
func internalGetConsumption(path string, typearg string) (int64, error) {
	...
	switch typearg {
	case "b":
		glog.V(3).Infof("Consumption for %s is %v", path, d.d_bcount * 512)
		return int64(d.d_bcount) * 512, nil
	case "i":
		glog.V(3).Infof("Inode consumption for %s is %v", path, d.d_icount)
